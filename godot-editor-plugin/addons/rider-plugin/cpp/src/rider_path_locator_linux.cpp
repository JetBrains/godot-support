...
std::optional<InstallInfo> RiderPathLocator::get_install_info_from_rider_path(const std::string &path,
    ...
    const fs::path cpp_plugin = rider_dir / "plugins" / "rider-cpp";
    if (!directory_exists_and_non_empty(cpp_plugin.string())) return std::nullopt;
...
std::set<InstallInfo, InstallInfoLess> RiderPathLocator::collect_all_paths() {
    ...
    for (auto &i : get_install_infos_from_toolbox(get_toolbox_path(), "Rider.sh")) s.insert(i);
