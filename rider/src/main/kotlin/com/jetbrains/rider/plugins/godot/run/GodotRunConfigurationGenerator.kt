...
class GodotRunConfigurationGenerator(project: Project) : ProtocolSubscribedProjectComponent(project) {
    ...
    init {
        project.solution.frontendBackendModel.hasGodotReference.whenTrue(projectComponentLifetime) {
            ...
            if (!runManager.allSettings.any { it.type is GodotDebugRunConfigurationType && it.name == PLAYER_CONFIGURATION_NAME }) {
                val configurationType =
                    ConfigurationTypeUtil.findConfigurationType(GodotDebugRunConfigurationType::class.java)
