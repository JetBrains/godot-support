...
func get_floor_angle(up_direction: Vector2) -> float:
	pass;
...
#desc print("Collided with: ", collision.collider.name)
#desc [/gdscript]
#desc [csharp]
#desc for (int i = 0; i < GetSlideCount(); i++)
#desc {
#desc KinematicCollision2D collision = GetSlideCollision(i);
#desc GD.Print("Collided with: ", (collision.Collider as Node).Name);
#desc }
#desc [/csharp]
#desc [/codeblocks]
