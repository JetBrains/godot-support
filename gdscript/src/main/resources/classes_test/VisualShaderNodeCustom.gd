...
virtual const func _get_code(input_vars: String[], output_vars: String[], mode: int, type: int) -> String:
	pass;
...
#desc You can customize the generated code based on the shader [param mode] (see [enum Shader.Mode]) and/or [param type] (see [enum VisualShader.Type]).
#desc Defining this method is [b]optional[/b].
virtual const func _get_func_code(mode: int, type: int) -> String:
	pass;
...
virtual const func _get_return_icon_type() -> int:
	pass;
...
virtual const func _is_available(mode: int, type: int) -> bool:
	pass;
