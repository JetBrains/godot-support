...
class GdInheritanceIdRefCompletionContributor : CompletionContributor() {
    init {
        extend(
            ...
            psiElement().withParent(GdInheritanceIdRef::class.java),
            GdInheritanceIdRefCompletionProvider(),
        )
    }
}

private class GdInheritanceIdRefCompletionProvider : PolySymbolsCompletionProviderBase<GdInheritanceIdRef>() {

    override fun getContext(position: PsiElement): GdInheritanceIdRef? =
        position.parent?.asSafely<GdInheritanceIdRef>()
