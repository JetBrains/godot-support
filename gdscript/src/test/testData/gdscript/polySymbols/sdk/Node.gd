...
const FLAG_PROCESS_THREAD_MESSAGES = 1
...
const FLAG_PROCESS_THREAD_MESSAGES_PHYSICS = 2
...
const DUPLICATE_USE_INSTANTIATION = 8
...
const AUTO_TRANSLATE_MODE_INHERIT = 0
...
const AUTO_TRANSLATE_MODE_ALWAYS = 1
...
var auto_translate_mode: int: get = get_auto_translate_mode, set = set_auto_translate_mode

## An optional description to the node. It will be displayed as a tooltip when hovering over the node in the editor's Scene dock.
var editor_description: String: get = get_editor_description, set = set_editor_description
...
var multiplayer: MultiplayerAPI: get = get_multiplayer
...
var name: StringName: get = get_name, set = set_name
...
var owner: Node: get = get_owner, set = set_owner
...
var physics_interpolation_mode: int: get = get_physics_interpolation_mode, set = set_physics_interpolation_mode
...
var process_mode: int: get = get_process_mode, set = set_process_mode
...
var process_physics_priority: int: get = get_physics_process_priority, set = set_physics_process_priority
...
var process_priority: int: get = get_process_priority, set = set_process_priority
...
var process_thread_group: int: get = get_process_thread_group, set = set_process_thread_group

## Change the process thread group order. Groups with a lesser order will process before groups with a greater order. This is useful when a large amount of nodes process in sub thread and, afterwards, another group wants to collect their result in the main thread, as an example.
var process_thread_group_order: int: get = get_process_thread_group_order, set = set_process_thread_group_order
...
var process_thread_messages: int: get = get_process_thread_messages, set = set_process_thread_messages
...
var scene_file_path: String: get = get_scene_file_path, set = set_scene_file_path
...
func call_deferred_thread_group(method: StringName, ...args: Array) -> Variant:
	pass

## This function ensures that the calling of this function will succeed, no matter whether it's being done from a thread or not. If called from a thread that is not allowed to call the function, the call will become deferred. Otherwise, the call will go through directly.
func call_thread_safe(method: StringName, ...args: Array) -> Variant:
	pass
...
func get_path_to(node: Node, use_unique_path: bool) -> NodePath:
	pass
...
func reparent(new_parent: Node, keep_global_transform: bool) -> void:
	pass
...
func rpc(method: StringName, ...args: Array) -> int:
	pass
...
## May return [constant OK] if the call is successful, [constant ERR_INVALID_PARAMETER] if the arguments passed in the [param method] do not match, [constant ERR_UNCONFIGURED] if the node's [member multiplayer] cannot be fetched (such as when the node is not inside the tree), [constant ERR_CONNECTION_ERROR] if [member multiplayer]'s connection is not available.
func rpc_id(peer_id: int, method: StringName, ...args: Array) -> int:
	pass
